...
# ==============================================================================
# FUNGSI-FUNGSI PLOTTING (DIREFAKTORISASI DARI FUNGSI HANDLE)
# ==============================================================================
...
def plot_wfh(kelamin, umur_anak, panjang_anak, berat_anak):
    ...
    try:
        ...
        if umur_anak <= 24:
            nama_file, x_axis_label, judul, x_col_name = f"wfl_{'girls' if kelamin == 'P' else 'boys'}_0-to-2-years_zscores.xlsx", "Panjang Badan (cm)", f'Grafik Berat Badan vs Panjang Badan - Anak {gender_text} (0-2 Tahun)', 'Length'
        else:
            nama_file, x_axis_label, judul, x_col_name = f"wfh_{'girls' if kelamin == 'P' else 'boys'}_2-to-5-years_zscores.xlsx", "Tinggi Badan (cm)", f'Grafik Berat Badan vs Tinggi Badan - Anak {gender_text} (2-5 Tahun)', 'Height'
...
def plot_hcfa(kelamin, umur_anak_bulan, hc_anak):
    ...
    try:
        ...
        st.error(f"Terjadi error saat membuat grafik HCFA: {e}")
...
st.sidebar.title("Posyandu Mawar Karang Baru")
st.sidebar.header("Input Data Anak")

# Input tunggal di sidebar
kelamin = st.sidebar.radio("Pilih Jenis Kelamin:", ('L', 'P'), format_func=lambda x: 'Laki-laki' if x == 'L' else 'Perempuan')
umur_anak = st.sidebar.number_input("Umur (bulan):", min_value=0.0, max_value=120.0, value=12.0, step=0.5)
berat_anak = st.sidebar.number_input("Berat (kg):", min_value=0.0, value=7.0, step=0.1)
panjang_anak = st.sidebar.number_input("Panjang/Tinggi (cm):", min_value=0.0, value=75.0, step=0.5)
hc_anak = st.sidebar.number_input("Lingkar Kepala (cm):", min_value=0.0, value=45.0, step=0.5)

if st.sidebar.button("Tampilkan Semua Kurva"):
    st.header("Hasil Analisis Pertumbuhan")
    
    # Memanggil setiap fungsi plotting
    plot_wfa(kelamin, umur_anak, berat_anak)
    st.markdown("---") # Garis pemisah
    plot_wfh(kelamin, umur_anak, panjang_anak, berat_anak)
    st.markdown("---")
    plot_bmi(kelamin, umur_anak, panjang_anak, berat_anak)
    st.markdown("---")
    plot_lhfa(kelamin, umur_anak, panjang_anak)
    st.markdown("---")
    plot_hcfa(kelamin, umur_anak, hc_anak)
